...
the same time, we get the algorithm in Listing~\ref{isqrt_heron_improved}, in which
the termination condition $a \le g(a)$ has been replaced with the equivalent
condition $a \le \floor{n/a}$.
...
near square root implementation shown in Listing~\ref{isqrt_recursive}, where the
...
recursive version shown in Listing~\ref{isqrt_recursive}. Instead, we unwind the recursion
to obtain an iterative version of the algorithm. This version is presented in
Listing~\ref{isqrt_iterative}.
...
equivalent to Listing~\ref{isqrt_recursive}. Rather than describing the equivalence and
...
The first loop invariant is $d = \floor{c/2^s}$; this should be
...
The iterative algorithm shown in Listing~\ref{isqrt_iterative} specialises easily
...
Listing~\ref{isqrt64}. While the original iterative algorithm was developed for
