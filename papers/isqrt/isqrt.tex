...
unbounded, it's desirable to have an implementation of integer square root
that remains reasonably efficient even for large inputs. However, the CPython
...
enough. The algorithm described in this paper hits the sweet spot for CPython,
giving a compromise between efficiency and simplicity.
...
of~$x$, we know that $2^{15} \le y \le 2^{16}$. But if $y = 2^{16}$ then it
...
To make the lookup table work, the key fact is that given a 16-bit
integer $t$ satisfying $2^{14} \le t < 2^{16}$, we can compute a near square
root of $t$ using only the most significant 8 bits of~$t$. The following lemma
shows that we have a small amount of choice in generating the lookup table
entries.

\begin{lemma} Suppose that $2^{14} \le t < 2^{16}$, and let $s = \floor{t /
256}$. Then $a := \csqrt{256(s+1)} - 1$ and $b := \csqrt{256s}$ are both near
square roots of~$t$.
\end{lemma}
