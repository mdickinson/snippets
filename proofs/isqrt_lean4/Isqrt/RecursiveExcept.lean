...
to the iterative `isqrtExcept` of `Isqrt.MonadicIsqrt`. The recursive algorithm,
...
`<<`) become the `Except`-returning `pyFloordivExcept` / `pyRshiftExcept` /
`pyLshiftExcept`, so every line that could raise in Python is a monadic bind `←`.
...
Correctness is proved in `Isqrt.RecursiveExceptCorrectness`.
-/
...
def isqrtAuxExcept (s : ℕ) (c n : Int) : Except PyException Int :=
  match s with
  | 0 => pure 1
  | s + 1 => do
    let k ← pyFloordivExcept (c - 1) 2
    let cHalf ← pyFloordivExcept c 2
    let nShift ← pyRshiftExcept n (2 * k + 2)
    let a ← isqrtAuxExcept s cHalf nShift
    let lsh ← pyLshiftExcept a k
    let rsh ← pyRshiftExcept n (k + 2)
    let q ← pyFloordivExcept rsh a
    pure (lsh + q)
...
computes `c = (n.bit_length() - 1) // 2`, calls `isqrtAuxExcept` with the
structural counter seeded at `c.bit_length()`, and applies the final `a-1`/`a`
adjustment. Correctness is `Isqrt.RecursiveExceptCorrectness`. -/
def isqrtRecExcept (n : Int) : Except PyException Int := do
  if n < 0 then
  ...
  let a ← isqrtAuxExcept (pyBitLength c).toNat c n
