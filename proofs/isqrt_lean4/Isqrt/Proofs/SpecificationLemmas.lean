...
(`∃ h : succeeded x, …(returnValue x h)` and its `failed`/`exception` twin),
discharging the success/failure proof and the `returnValue`/`exception` extraction
...
theorem returnValue_satisfies {ε α : Type _} {x : Except ε α} {a : α} (hx : x = .ok a)
    (p : α → Prop) (hp : p a) : ∃ h : succeeded x, p (returnValue x h) := by
  subst hx; exact ⟨True.intro, hp⟩

/-- From `x = .error e` and `q e`: the computation `x` failed, and its raised
exception satisfies `q`. The `failed`/`exception` twin of `returnValue_satisfies`. -/
theorem exception_satisfies {ε α : Type _} {x : Except ε α} {e : ε} (hx : x = .error e)
    (q : ε → Prop) (hq : q e) : ∃ h : failed x, q (exception x h) := by
