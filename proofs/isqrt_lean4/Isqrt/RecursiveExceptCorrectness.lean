/-
Correctness of the recursive monadic integer square root `isqrtRecExcept`.

Strategy: structural induction on the counter `s` for `isqrtAuxExcept`, carrying
...
Each inductive step discharges the `.ok`-ness of every monadic operation (the
obligation absent in the proof-carrying recursive proof `Isqrt.Correctness`) and
then reuses the *same* algebra as that proof — `key_isqrt_lemma` applied to the
recursive subproblem's value. The top-level result `isqrtRecExcept_eq_ok_iff` is
a faithful total spec, mirroring the iterative `isqrtExcept_eq_ok_iff`.
-/
...
private theorem isqrtAuxExcept_correctness :
    ∀ (s : ℕ) {c n : ℤ}, 0 ≤ c → 0 < n →
      ...
      ∃ a, isqrtAuxExcept s c n = .ok a ∧ 0 < a ∧ isNearSquareRoot a n := by
  intro s
  induction s with
  | zero =>
    ...
    -- `isqrtAuxExcept 0 0 n = .ok 1`, and `1 ≤ n < 4` gives the near-√ property.
  ...
  | succ s ih =>
    ...
    have hred : isqrtAuxExcept (s + 1) c n
        = .ok (a * 2 ^ k.toNat + Int.fdiv (Int.fdiv n (2 ^ (k + 2).toNat)) a) := by
      unfold isqrtAuxExcept
      simp only [pyFloordivExcept_eq_ok (show (2 : ℤ) ≠ 0 by norm_num),
        ← hk_def, ← hd_def, Except.ok_bind',
        pyRshiftExcept_eq_ok h2k2_nn, ← hm_def, ha_eq,
        pyLshiftExcept_eq_ok k_nn, pyRshiftExcept_eq_ok hk2_nn,
        pyFloordivExcept_eq_ok (ne_of_gt a_pos)]
      rfl
...
`do`-block to the `isqrtAuxExcept` call characterised by `isqrtAuxExcept_correctness`
...
to the iterative `isqrtExcept_eq_ok_iff`. -/
theorem isqrtRecExcept_eq_ok_iff (n : ℤ) :
    (match isqrtRecExcept n with
  ...
  · -- n < 0: the first guard raises, short-circuiting the `do` block.
    have herr : isqrtRecExcept n
        = .error (.valueError "isqrt() argument must be nonnegative") := by
      unfold isqrtRecExcept; rw [if_pos hneg]; rfl
    rw [herr]; exact ⟨hneg, rfl⟩
  · -- n = 0: special-cased to 0.
    subst hzero
    have hok : isqrtRecExcept 0 = .ok 0 := by unfold isqrtRecExcept; norm_num; rfl
  ...
  · -- 0 < n: the recursion runs and never raises.
    ...
    obtain ⟨a, ha_eq, _a_pos, a_near⟩ :=
      isqrtAuxExcept_correctness (pyBitLength c).toNat
        (c := c) (n := n) (isqrt_c_nonneg hn0) hpos rfl (size_condition_initial hpos)
    have hred : isqrtRecExcept n = .ok (if n < a * a then a - 1 else a) := by
      conv_lhs => unfold isqrtRecExcept
      simp only [if_neg (show ¬ n < 0 by omega), if_neg hn0, pure_bind,
        pyFloordivExcept_eq_ok (show (2 : ℤ) ≠ 0 by norm_num), ← hc_def]
