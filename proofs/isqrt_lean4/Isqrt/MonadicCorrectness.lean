...
body of `isqrtExcept` lifted out: it reads `e = d` (the previous shift), recomputes
...
def stepM (c n : ℤ) (r : MProd ℤ ℤ) (s : ℤ) : Except PyException (MProd ℤ ℤ) := do
  let dNew ← pyRshiftExcept c s
  let lsh ← pyLshiftExcept r.fst (dNew - r.snd - 1)
  let rsh ← pyRshiftExcept n (2 * c - r.snd - dNew + 1)
  let q ← pyFloordivExcept rsh r.fst
  pure ⟨lsh + q, dNew⟩

/-- A `forIn` whose body always yields the result of a monadic step `g` is a `foldlM`
over the same list. The monadic analogue of `List.forIn'_pure_yield_eq_foldl`'s use in
the iterative proof, specialised to the "always yield" shape the `do` block produces. -/
...
the monadic counterpart of `foldl_reverseRange_invariant`. Each step must additionally
witness that `g x s` takes its `.ok` branch (the obligation absent in the pure `Id` case).
...
theorem stepM_eq_ok {c n : ℤ} (r : MProd ℤ ℤ) (s : ℤ)
  ...
  simp only [stepM, pyRshiftExcept_eq_ok hs_nn, Except.ok_bind,
    pyLshiftExcept_eq_ok hK, pyRshiftExcept_eq_ok hJ,
    pyFloordivExcept_eq_ok (ne_of_gt ha_pos)]
  rfl

/-- The monadic loop's `foldlM` is `.ok`, and its running approximation is a positive
near square root of `n`. The `Except` counterpart of `loopFold_near`: a position-indexed
`foldlM` invariant whose motive carries the running `a > 0`, the threaded shift
`d = c >> s`, and the near-√ property `isNearSquareRoot a ⌊n / 4^(c - c>>s)⌋`. -/
...
/-- Correctness of the monadic integer square root `isqrtExcept`.
...
theorem isqrtExcept_eq_ok_iff (n : ℤ) :
    (match isqrtExcept n with
  ...
  · -- n < 0: the first guard raises, short-circuiting the `do` block.
    have herr : isqrtExcept n = .error (.valueError "isqrt() argument must be nonnegative") := by
      unfold isqrtExcept; rw [if_pos hneg]; rfl
    rw [herr]; exact ⟨hneg, rfl⟩
  · -- n = 0: special-cased to 0.
    subst hzero
    have hok : isqrtExcept 0 = .ok 0 := by unfold isqrtExcept; norm_num; rfl
  ...
  · -- 0 < n: the loop runs and never raises.
    ...
    have hred : isqrtExcept n = .ok (y.fst - if y.fst * y.fst > n then 1 else 0) := by
      conv_lhs => unfold isqrtExcept
      simp only [if_neg (show ¬ n < 0 by omega), if_neg hn0, pure_bind,
        pyFloordivExcept_eq_ok (show (2 : ℤ) ≠ 0 by norm_num)]
