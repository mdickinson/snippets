...
- `Isqrt/PythonOps.lean` — `pyFloordiv`, `pyRshift`, `pyLshift`, `pyBitLength`, and the `py//`, `py>>`, `py<<` operators. Each Python operator that can raise on bad input is a proof-carrying function: the precondition (`b ≠ 0`, `0 ≤ k`) is an argument with a `by omega` default. Operator precedences are engineered to match Python's.
- `Isqrt/FDivLemmas.lean`, `Isqrt/BitLengthLemmas.lean` — arithmetic lemmas about `Int.fdiv` (Python's floor division) and `pyBitLength`. Thin wrappers over Mathlib + `Int.fdiv_eq_ediv_of_nonneg`.
